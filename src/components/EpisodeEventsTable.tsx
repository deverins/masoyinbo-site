...
const EpisodeEventsTable: React.FC<{ episodeId: string; onEdit: any }> = ({ episodeId, onEdit }) => {
  ...
  const fetchEpisodeData = async (episodeId: string) => {
    try {
      ...
      setEpisodeLink(data.episode?.episodeLink);
      setParticipantFullName(data.participantFullName);
      setTotalAmountAvailable(data.episode?.availableAmountToWin || 0);
    } catch (error) {
  ...
  const handleSave = async () => {
    try {
      const episodeId = localStorage.getItem('paramsEpisodeId');
      if (!episodeId) {
        return;
      }
      ...
      setEvents([]);
      toast.success("Episode events create successfully")
      navigate.push(`/episode/${episodeId}`)
    } catch (error) {
    ...
    <>
      <div className="flex flex-col items-center">
        ...
        <div className="mt-4 w-full max-w-[560px] mb-10">
          ...
          <div className="dark:text-neutral-300 font-semibold capitalize">Participant Name: {participantFullName}</div>
