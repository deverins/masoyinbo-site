...
const ParticipationUsers = () => {
  ...
  useEffect(() => {
    const fetchRequestPool = async () => {
      try {
        const { data } = await axios.get(`${API_URL}/v1/api/get-Completed-participants`);
  ...
  return (
    <main className="p-4">
      <h1 className="text-2xl font-bold text-center mb-8 mt-8 text-red-500">All Participant Users</h1>
